...
# No limit on number of drinks
def order_now(menu):
    '''
        This function takes the order of the customer
        Calculate the price of their drinks (with possible discounts)
        Inputs: Takes menu, a dictionary containing drink names (keys) and prices (values)
        Outputs: Returns total price, a float, of the customer's order
    '''
    order_list = []
    print("This is our current menu: ")
    print(menu)
...
def main():
    ...
    menu = add_item(menu)
    menu = update_item(menu)
    print("Customer perspective")
    total = order_now(menu) # float representing customer total
    print(total)
